...
Similarly, `emperor_count <- emperor_count + 1` increases by one after processing each row, thus producing the count of emperors.
...
Data file [`elections.txt`](https://raw.githubusercontent.com/codingsocialscience/data/refs/heads/master/elections.txt) contains candidates names, their parties, their vote counts and ther birth year so that each candidate is on a single row:
...
1. Calculate the number of candidates in PartyA.

```{r}

```

2. Calculate the number of votes candidates in PartyA recieved.

```{r}

```

3. On avarage, how many votes did candidates in PartyA recieve?

```{r}

```

4. On avarage, did PartyB get more votes than PartyA per candidate?
